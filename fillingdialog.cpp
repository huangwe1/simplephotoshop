...
{
    *type = photoset::Ellipse;
}
void FillingDialog::setBrushStyle()
{
    int i = ui->comboBox->currentIndex();
    brush->setStyle(Qt::BrushStyle(Qt::SolidPattern + i < 6? (i - 2) * 2 : (i + 3)));
}
...
{
     QColor newColor = QColorDialog::getColor(Qt::white,NULL,"pick Color",QColorDialog::ShowAlphaChannel);
    if (newColor.isValid())
